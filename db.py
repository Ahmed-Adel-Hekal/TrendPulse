...
def get_user_settings(uid):
    ...
    if row:
        d = dict(row)
        # Ensure ui_language key always present
...
def quota_ok_atomic(user) -> bool:
    """Atomically check AND record quota consumption. Returns True if quota was available."""
    uid   = user["id"]
    ...
    with get_conn() as conn:
        ...
        if used >= limit:
            return False
        # Do NOT insert here — insert only on success in update_generation
        return True
...
def create_scheduled_generation(uid, topic, content_type, platforms, language,
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            "VALUES (?,?,?,?,'static' ,?,'scheduled',?,?,?)" if False else
            "INSERT INTO generations "
            "(id,user_id,topic,content_type,platforms,language,status,config_json,scheduled_at,created_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?)",
            (gid, uid, topic, content_type, json.dumps(platforms), language,
...
def get_generation_admin(gid):
    """Get generation without user ownership check (admin use)."""
    with get_conn() as conn:
