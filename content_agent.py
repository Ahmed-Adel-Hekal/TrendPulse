...
"""
    ...
    def generate(self, topic, comp_insight=None, trend_insight=None, brand_block=None, features_block=None):
        ...
        # ── FIX: write result.txt only when DEBUG=true, not in production ──
...
def _generate_one_idea(idx, topic, content_type, config, comp_insight, trend_insight, brand_block, features_block):
    """
    Generate a single idea. Returns (idx, idea_dict) or (idx, None) on failure.

    FIX: Previously a parse failure here would propagate as an unhandled exception
    to run_content_pipeline, which caught it and returned status='failed' — even
    though the LLM DID respond (the raw text was saved in result.txt).  Now we:
      1. Log the raw LLM output on parse failure for debugging.
      2. Return the fallback idea instead of None so the pipeline can continue.
    """
    import logging as _logging
    _log = _logging.getLogger("ContentAgent")
    try:
        one_config = AgentConfig(
            llm_provider=config.llm_provider, llm_api_key=config.llm_api_key,
            video_content=config.video_content, images=config.images,
            brand_color=config.brand_color, brand_img=config.brand_img,
            target_platform=config.target_platform, model=config.model,
            language=config.language, number_idea=1
        )
        agent = ContentAgent(config=one_config)
        raw   = agent.generate(
            topic, comp_insight=comp_insight, trend_insight=trend_insight,
            brand_block=brand_block, features_block=features_block
        )
        if not raw:
        ...
        # ── FIX: catch JSON parse errors gracefully; log raw for debugging ──
        try:
            ...
            # Try a second time with a stripped version of the raw text
            try:
                import json, re
...
def run_content_pipeline(topic, platforms, content_type, language, brand_color, brand_img,
    ...
    import logging as _logging
    _log = _logging.getLogger("ContentAgent.pipeline")
    try:
        ...
        config = AgentConfig(
            ...
            video_content=(content_type=="video"), images=True,
            brand_color=brand_color, brand_img=brand_img,
            target_platform=platform_literals,
            model=llm_model or "gemini-2.5-flash",
            language=language, number_idea=1
        )
        ...
        with ThreadPoolExecutor(max_workers=min(n, 4)) as pool:
            futures = {
                pool.submit(
                    _generate_one_idea, i, topic, content_type, config,
                    comp_insight, trend_insight or "", brand_block or "", features_block or ""
                ): i
                for i in range(n)
            }
            for fut in _as_completed(futures):
                try:
                    ...
                    idx = futures[fut]
                    _log.warning("Future for idea %d raised: %s", idx, e)
                    idea = None

                if idea is not None:
                    ideas_by_idx[idx] = idea
                else:
                    # ── FIX: use fallback rather than letting the whole pipeline fail ──
        ...
        results = run_media_generation(
            parsed=parsed, content_type=content_type, language=language,
            brand_color=brand_color, image_url=image_url, aspect_ratio=aspect_ratio,
            out_dir=output_dir, image_model=image_model, video_model=video_model,
            llm_api_key=llm_api_key, image_api_key=image_api_key,
            video_api_key=video_api_key, warnings=warnings
        )
